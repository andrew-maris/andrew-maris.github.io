...
The boundary also provides evidence for a theorized resistive-ballooning-mode destabilization mechanism, so the result is not purely empirical. And because burning plasmas have naturally low edge collisionality from self-heating, the scaling suggests they may be able to operate at super-Greenwald densities.

## Closing the loop

An accurate stability metric is only interesting if a machine can act on it. I earned PhD run time at DIII-D to test exactly that. The "DL Supervisor" control scheme regulates the machine-learned risk metric in real time by lowering the density target or raising neutral-beam heating.

Density-limit disruptions were reproducibly suppressed. Applying an analogous H-mode metric during a current ramp-down also avoided an H/L back-transition. These are the first demonstrations of real-time density-limit avoidance using machine-learned risk metrics.
